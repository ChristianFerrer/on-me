...
export default async function InicioPage() {
  ...
  return (
    // `svh`, no `dvh` -mismo motivo que Screen.tsx-: a Safari iOS no se le
    // puede confiar el valor de `dvh` en la primera pintura, y `svh` nunca
    // sobreestima el área visible real, así que el fondo nunca deja ver el
    // body claro por debajo.
    <div className="aurora-night min-h-svh w-full text-chalk" lang={locale}>
      <main className="mx-auto flex min-h-svh w-full max-w-[30rem] flex-col gap-6 px-5 pt-[max(1.75rem,env(safe-area-inset-top))] pb-[max(1.25rem,env(safe-area-inset-bottom))] sm:max-w-[34rem] sm:justify-center sm:pt-10 lg:max-w-[40rem] xl:max-w-[44rem]">
